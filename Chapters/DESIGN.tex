...
\end{flushright}
\end{quote}
% The previous survey motivates a small, open,
% statically partitioned hypervisor as the consolidation mechanism. We now
% translate those findings into a concrete architecture suitable for
% video-surveillance UAVs under tight SWaP-C: the \gls{sspfs} -- 
% a flight stack that leverages the Bao hypervisor for video-surveillance missions, where resource-intensive but non-critical streaming coexists with safety-critical flight control on consolidated hardware.

% We begin by defining requirements and constraints for the target application,
% then analyze the conventional architecture and its drawbacks. We implement an
...
Next, we select the hardware -- the \gls{uav} and the \gls{uavic} -- and map
