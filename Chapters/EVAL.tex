...
\gls{sspfs}.
%
In the \emph{unsupervised} case, malicious components execute directly on the host
...
%
In the \emph{supervised} case, we deploy malicious components solely to the Companion
...
\begin{figure}[!hbtp]
  \centering
  \includegraphics[width=0.7\textwidth]{./img/pdf/uav-main-eval-funcTest}
...
% Preamble (once, if not already)
% \usepackage{booktabs}
% \usepackage{array}
% \usepackage{multirow}
% \usepackage[table]{xcolor} % if using colored status macros like \Fail, \Impact, etc.

% (Optional) status macros used in your example:
% \newcommand{\Run}[1]{\cellcolor{green!10}{\textbf{#1}}}
% \newcommand{\Impact}[1]{\cellcolor{orange!15}{\textbf{#1}}}
% \newcommand{\Fail}[1]{\cellcolor{red!10}{\textbf{#1}}}
% \newcommand{\Neutral}[1]{#1}
...
Table~\ref{tab:func-val-priv} summarizes functional validation results by privilege level and failure mode for each system. 
\emph{P}, \emph{C}, \emph{M}, \emph{L}, and \emph{B} denote PX4, Companion, Malicious, Linux, and Bao, respectively. 
Cell colors indicate the execution status: 
\hlighthex{E8F5E9}{1B5E20}{\textbf{running}}, 
\hlighthex{FFF3E0}{E65100}{\textbf{heavy impact}}, and 
\hlighthex{FDEAEA}{B00020}{\textbf{failure}}.
Results show that, under supervision (\gls{sspfs}), faults are contained within
the non-critical stack, whereas the unsupervised baseline (\gls{uspfs}) exhibits
propagation to critical components.

\begin{table}[!htbp]
  \centering
  \caption{Functional validation by privilege level and failure mode for each system}
  \label{tab:func-val-priv}
  ...
  \begin{tabular}{@{}%
    >{\raggedright\arraybackslash}p{0.10\textwidth} % System
    >{\centering\arraybackslash}p{0.044\textwidth}  % Priv Panic P
    >{\centering\arraybackslash}p{0.044\textwidth}  % Priv Panic C
    >{\centering\arraybackslash}p{0.044\textwidth}  % Priv Panic M
    >{\centering\arraybackslash}p{0.044\textwidth}  % Priv MemCor P
    >{\centering\arraybackslash}p{0.044\textwidth}  % Priv MemCor C
    >{\centering\arraybackslash}p{0.044\textwidth}  % Priv MemCor M
    >{\centering\arraybackslash}p{0.044\textwidth}  % Priv MemExh P
    >{\centering\arraybackslash}p{0.044\textwidth}  % Priv MemExh C
    >{\centering\arraybackslash}p{0.044\textwidth}  % Priv MemExh M
    >{\centering\arraybackslash}p{0.044\textwidth}  % Priv Killinit P
    >{\centering\arraybackslash}p{0.044\textwidth}  % Priv Killinit C
    >{\centering\arraybackslash}p{0.044\textwidth}  % Priv Killinit M
    >{\centering\arraybackslash}p{0.044\textwidth}  % Unpriv MPCIO P
    >{\centering\arraybackslash}p{0.044\textwidth}  % Unpriv MPCIO C
    >{\centering\arraybackslash}p{0.044\textwidth}  % Unpriv MPCIO M
  @{}}
    \toprule
    \multicolumn{1}{c}{} &
    \multicolumn{12}{c}{\textbf{Privileged}} &
    \multicolumn{3}{c}{\textbf{Unprivileged}} \\
    \cmidrule(lr){2-13}\cmidrule(l){14-16}
    \textbf{System} &
    \multicolumn{3}{c}{\textbf{Panic}} &
    \multicolumn{3}{c}{\textbf{Mem corruption}} &
    \multicolumn{3}{c}{\textbf{Mem exhaust}} &
    \multicolumn{3}{c}{\textbf{Kill \texttt{init}}} &
    \multicolumn{3}{c}{\textbf{Mem+Proc+CPU/IO}} \\
    \midrule

    % ----------------------- USPFS (2 rows) -----------------------
    \multirow{2}{*}{\textbf{USPFS}} &
      \Fail{P} & \Fail{C} & \Fail{M} &
      \Fail{P} & \Fail{C} & \Fail{M} &
      \Fail{P} & \Fail{C} & \Fail{M} &
      \Fail{P} & \Fail{C} & \Fail{M} &
      \Impact{P} & \Impact{C} & \Impact{M} \\
    & % sub-row with L under each PCM cell
      \Fail{} & \Fail{L} & \Fail{} &
      \Fail{} & \Fail{L} & \Fail{} &
      \Fail{} & \Fail{L} & \Fail{} &
      \Fail{} & \Fail{L} & \Fail{} &
      \Impact{} & \Impact{L} & \Impact{} \\
    \midrule

    % ----------------------- SSPFS (3 rows) -----------------------
    \multirow{3}{*}{\textbf{SSPFS}} &
      \Run{P} & \Fail{C} & \Fail{M} &
      \Run{P} & \Fail{C} & \Fail{M} &
      \Run{P} & \Fail{C} & \Fail{M} &
      \Run{P} & \Fail{C} & \Fail{M} &
      \Run{P} & \Impact{C} & \Impact{M} \\
    & % L row: P has its own L; C+M share one merged L
      \Run{L} & \multicolumn{2}{c}{\Fail{L}} &
      \Run{L} & \multicolumn{2}{c}{\Fail{L}} &
      \Run{L} & \multicolumn{2}{c}{\Fail{L}} &
      \Run{L} & \multicolumn{2}{c}{\Fail{L}} &
      \Run{L} & \multicolumn{2}{c}{\Impact{L}} \\
    & % B row: same merge pattern for C+M
     \Run{} & \Run{B} & \Run{}  &
     \Run{} & \Run{B} & \Run{} &
     \Run{} & \Run{B} & \Run{} &
     \Run{} & \Run{B} & \Run{} &
  ...
  \begin{tabular}{@{}l l l l l@{}}
    \Run{\phantom{XX}}\;Running
    & \Impact{\phantom{XX}}\;Heavy impact
    & \Fail{\phantom{XX}}\;Fail
    & \quad P: PX4 \quad C: Companion \quad M: Malicious \quad L: Linux \quad B: Bao
  \end{tabular}
...
indicate degradation and negative values indicate improvement. \gls{sspfs} shows
...
placement in the \gls{sspfs} configuration via
\lstinline[keepspaces=true]|place_phys = true|.

\vspace{1\baselineskip}   % two lines worth of vertical space
...
outliers with over 30\% improvement, while cache coloring again has limited impact
