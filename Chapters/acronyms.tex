%!TEX root = ../template.tex
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% acronyms.tex
%% NOVA thesis document file
%%
%% Acronyms definition
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\typeout{NT FILE acronyms.tex}%


% \newacronym{abbrev}{abbrev}{abbreviation of a longer text}
% \newacronym{aaa}{aaa}{acronym aaa}
% \newacronym{aab}{aab}{acronym aab}
% \newacronym{aba}{aba}{acronym aba}
% \newacronym{bbb}{bbb}{acronym bbb}
% \newacronym{xpt}{xpto}{and extension of a xpto xpto xpto xpto xpto xpto xpto xpto xpto xpto xpto xpto xpto xpto xpto xpto xpto xpto xpto}

% \newacronym{novathesis}{\emph{novathesis}}{\emph{novathesis} template}
%
% \newacronym{novathesisclass}{\texttt{novathesis.cls}}{\texttt{novathesis.cls} class}


\newacronym[sort=novathesis]{novathesis}{\novathesis}{NOVAthesis \LaTeX}

\newacronym{novathesisclass}{novathesis.cls}{\novathesis\ class}

\newacronym{NOVA}{NOVA}{NOVA University Lisbon}

\newacronym{FCT}{FCT}{NOVA School of Science and Technology}

\newacronym{DI}{DI}{Department of Computer Science}

% ============ MINE ======================
% here are the acronym entries
\newacronym{mei}{MEI}{Mestrado em Engenharia Inform\'{a}tica}
\newacronym{um}{UM}{Universidade do Minho}
\newacronym{soa}{SOA}{Service Oriented Architecture}
%
\newacronym{oop}{OOP}{Object Oriented Programming}
\newacronym{oose}{OOSE}{Object Oriented Software Engineering}
\newacronym{omt}{OMT}{Object-Modeling Technique}
\newacronym{vdi}{VDI}{Verein Deutscher Ingenieure}
\newacronym{api}{API}{Application Programming Interface}
\newacronym{pwm}{PWM}{Pulse-Width Modulation}
\newacronym{ic}{IC}{Integrated Circuit}
\newacronym{ssr}{SSR}{Solid-State Relay}
\newacronym{pid}{PID}{Proportional Integrative Derivative}
\newacronym{pci}{PCI}{Peripheral Component Interconnect}
\newacronym{pcie}{PCIe}{Peripheral Component Interconnect Express}
\newacronym{pcb}{PCB}{Printed Circuit Board}
%
\newacronym{lvm}{LVM}{Linux Virtual Machine}
\newacronym{cli}{CLI}{Command Line Interface}
\newacronym{ui}{UI}{User Interface}
%
%
\newacronym{erd}{ERD}{Entity Relationship Diagram}
\newacronym{ddl}{DDL}{Data Definition Language}
\newacronym{dml}{DML}{Data Manipulation Language}
\newacronym{sql}{SQL}{Structured Query Language}
\newacronym{er}{ER}{Entity-Relationship}
\newacronym{tcp}{TCP}{Transmission Control Protocol}
\newacronym{ip}{IP}{Internet Protocol}
\newacronym{tcp-ip}{TCP/IP}{Transmission Control Protocol/Internet Protocol}
\newacronym{db}{DB}{Database}
\newacronym{bsp}{BSP}{Board Support Package}
\newacronym{rdbms}{RDBMS}{Relational Database Management System}
\newacronym{dbms}{DBMS}{Database Management System}
%
%%%%%%%%%%%%%%%%%%%%%% MIEEIC Thesis %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newacronym{hw}{HW}{hardware}
\newacronym{sw}{SW}{software}
\newacronym{uav}{UAV}{Unmanned Aerial Vehicle}
\newacronym{sph}{SPH}{Static Partitioning Hypervisor}
\newacronym{ota}{OTA}{Over-The-Air}
\newacronym{asil}{ASIL}{Automotive Safety and Integrity Level}
\newacronym{dal}{DAL}{Design/Development Assurance Level}
\newacronym{sil}{SIL}{Safety Integrity Level}
\newacronym{mcs}{MCS}{Mixed-Criticality System}
\newacronym{swap-c}{SWaP-C}{Size, Weight, Power and Cost}
\newacronym{wcet}{WCET}{Worst-Case Execution Time}
\newacronym{os}{OS}{Operating System}
\newacronym{rt}{RT}{Real-Time}
\newacronym{cpu}{CPU}{Central Processing Unit}
\newacronym{vm}{VM}{Virtual Machine}
\newacronym{rtos}{RTOS}{Real-Time Operating System}
\newacronym{gpos}{GPOS}{General-Purpose Operating System}
\newacronym{qos}{QoS}{Quality of Service}
\newacronym{vmm}{VMM}{Virtual Machine Monitor}
\newacronym{mmu}{MMU}{Memory Management Unit}
\newacronym{iommu}{IOMMU}{Input/Output Memory Management Unit}
\newacronym{io}{I/O}{Input/Output}
\newacronym{tcb}{TCB}{Trusted Computing Base}
\newacronym{llc}{LLC}{Last-Level Cache}
\newacronym{dos}{DoS}{Denial of Service}
\newacronym{isa}{ISA}{Instruction Set Architecture}
\newacronym{tee}{TEE}{Trusted Execution Environment}
\newacronym{tlb}{TLB}{Translation Lookaside Buffer}
\newacronym{gic}{GIC}{Generic Interrupt Controller}
\newacronym{gps}{GPS}{Global Positioning System}
\newacronym{uas}{UAS}{Unmanned Aerial System}
\newacronym{uv}{UV}{Unmanned Vehicle}
\newacronym{ugv}{UGV}{Unmanned Ground Vehicle}
\newacronym{usv}{USV}{Unmanned Surface Vehicle}
\newacronym{uuv}{UUV}{Unmanned Under water Vehicle}
\newacronym{utm}{UTM}{UAS Traffic Management}
\newacronym{faa}{FAA}{Federal Aviation Administration}
\newacronym{nasa}{NASA}{National Aeronautics and Space Administration}
\newacronym{bvlos}{BVLOS}{Beyond Visual Line-Of-Sight}
\newacronym{vtol}{VTOL}{Vertical Take-Off and Landing}
\newacronym{lap}{LAP}{Low Altitude Platform}
\newacronym{hap}{HAP}{High Altitude Platform}
\newacronym{hfc}{HFC}{Hydrogen Fuel Cell}
\newacronym{lipo}{LiPo}{Lithium Polymer}
\newacronym{sdk}{SDK}{Software Development Kit}
\newacronym{pv}{PV}{Photovoltaic}
\newacronym{gcs}{GCS}{Ground Control Station}
\newacronym{imu}{IMU}{Inertial Measuring Unit}
\newacronym{gnss}{GNSS}{Global Navigation Satellite System}
\newacronym{tof}{TOF}{Time-of-Flight}
\newacronym{bldc}{BLDC}{Brushless Direct Current}
\newacronym{esc}{ESC}{Electronic Speed Controller}
\newacronym{ac}{AC}{Alternating Current}
\newacronym{dc}{DC}{Direct Current}
\newacronym{fcs}{FCS}{Flight Control System}
\newacronym{rc}{RC}{Radio Controller}
\newacronym{rid}{RID}{Remote IDentifier}
\newacronym{ros}{ROS}{Robotic Operating System}
\newacronym{ddos}{DDoS}{Distributed Denial Of Service}
\newacronym{sdr}{SDR}{Software-Defined Radio}
\newacronym{fpv}{FPV}{First-Person View}
\newacronym{cad}{CAD}{Computer-Aided Design}
\newacronym{los}{LOS}{Line-Of-Sight}
\newacronym{osh}{OSH}{Open-Source Hardware}
\newacronym{oss}{OSS}{Open-Source Software}
\newacronym{cots}{COTS}{Commercial Off-The-Shelf}
\newacronym{bom}{BOM}{Bill Of Materials}
\newacronym{hdl}{HDL}{Hardware Description Language}
\newacronym{fmu}{FMU}{Flight Management Unit}
\newacronym{i2c}{I2C}{Inter-Integrated Circuit}
\newacronym{can}{CAN}{Controller Area Network}
\newacronym{uart}{UART}{Universal Asynchronous Receiver-Transmitter}
\newacronym{spi}{SPI}{Serial Peripheral Interface}
\newacronym{ram}{RAM}{Random Access Memory}
\newacronym{sram}{SRAM}{Static Random Access Memory}
\newacronym{bsd}{BSD}{Berkeley Software Distribution}
\newacronym{gpl}{GPL}{GNU Public License}
\newacronym{mcu}{MCU}{Micro Controller Unit}
\newacronym{usb}{USB}{Universal Serial Bus}
\newacronym{adc}{ADC}{Analog to Digital Converter}
\newacronym{fpga}{FPGA}{Field-Programmable Gate Array}
\newacronym{soc}{SoC}{System on Chip}
\newacronym{sd}{SD}{Storage Disk}
\newacronym{csi}{CSI}{Camera Serial Interface}
\newacronym{gsi}{GSI}{Generic Serial Interface}
\newacronym{udp}{UDP}{User Datagram Protocol}
\newacronym{lte}{LTE}{Long-Term Evolution}
\newacronym{vpn}{VPN}{Virtual Private Network}
\newacronym{rtc}{RTC}{Real-Time Communication}
\newacronym{csp}{CSP}{Context Security Policy}
\newacronym{rtps}{RTPS}{Real-Time Publish Subscribe}
\newacronym{hal}{HAL}{Hardware Abstraction Layer}
\newacronym{eeprom}{EEPROM}{Electrically Erasable Programmable Read-Only Memory}
\newacronym{abi}{ABI}{Application Binary Interface}
\newacronym{xml}{XML}{eXtensible Markup Language}
\newacronym{lan}{LAN}{Local Area Network}
\newacronym{uspfs}{USPFS}{Unsupervised Single-Platform Flight Stack}
\newacronym{sspfs}{SSPFS}{Supervised Single-Platform Flight Stack}
\newacronym{umpfs}{UMPFS}{Unsupervised Multi-Platform Flight Stack}
\newacronym{uavic}{UAVIC}{Unmanned Aerial Vehicle Integrated Controller}
\newacronym{diy}{DIY}{Do It Yourself}
\newacronym{swd}{SWD}{Serial Wire Debug}
\newacronym{gpu}{GPU}{Graphics Processing Unit}
\newacronym{ieee}{IEEE}{Institute of Electrical and Electronics Engineers}
\newacronym{gpio}{GPIO}{General Purpose Input/Output}
\newacronym{dma}{DMA}{Direct Memory Access}
\newacronym{cma}{CMA}{Contiguous Memory Allocation}
\newacronym{hd}{HD}{High-Definition}
\newacronym{fps}{FPS}{Frames Per Second}
\newacronym{mbps}{Mbps}{Megabits per second}
\newacronym{atf}{ATF}{Arm Trusted Firmware}
\newacronym{dtb}{DTB}{Device Tree Blob}
\newacronym{dts}{DTB}{Device Tree Source}
\newacronym{rtp}{RTP}{Real-time Transport Protocol}
\newacronym{rtsp}{RTSP}{Real-Time Streaming Protocol}
\newacronym{gui}{GUI}{Graphical User Interface}
\newacronym{ssh}{SSH}{Secure Shell}
\newacronym{oom}{OOM}{Out Of Memory}
\newacronym{vma}{VMA}{Virtual Memory Area}
\newacronym{pmu}{PMU}{Performance Monitor Unit}
\newacronym{vcpu}{vCPU}{Virtual CPU}
\newacronym{aics}{AICS}{Automotive and Industrial Control Suite}
\newacronym{lidar}{LiDAR}{Light Detection And Ranging}
\newacronym{uorb}{uORB}{Micro Object Request Broker}
\newacronym{emmc}{eMMC}{embedded Multi Media Card}
\newacronym{npu}{NPU}{Neural Processing Unit}
\newacronym{ai}{AI}{Artificial Intelligence}
\newacronym{tops}{TOPS}{Trillion Operations Per Second}
\newacronym{lcd}{LCD}{Liquid Crystal Display}
\newacronym{ide}{IDE}{Integrated Development Environment}
\newacronym{ndaa}{NDAA}{National Defense Authorization Act}
\newacronym{fifo}{FIFO}{First In, First Out}
\newacronym{sitl}{SITL}{Software in the Loop}
\newacronym{hitl}{HITL}{Hardware in the Loop}
\newacronym{plic}{PLIC}{Platform-Level Interrupt Controller}
\newacronym{aia}{AIA}{Advanced Interrupt Architecture}
\newacronym{ipc}{IPC}{Inter-Process Communication}

